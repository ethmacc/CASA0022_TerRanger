...
class _SampleDetailState extends State<SampleDetail> {
  ...
  List parseAndScalePts(String rawData) {
    List<int> dataList = json.decode(rawData).cast<int>().toList();
    List<Vector3> scaledList = [];
    List pointList = [];
    List<double> accData = [];
    ...
    debugPrint('scaled vectors: $scaledList');
      ...
      body: FutureBuilder(
        ...
        builder: (context, allSamples) {
          ...
          if (samplesToLoad.isNotEmpty) {
            ...
            List pointListData = parseAndScalePts(selectedSample['tofData']); 
            ...
            return Center(
              ...
              children: [
                SizedBox(
                  ...
                  child:  ScatterChart(
                    ScatterChartData(
                      scatterSpots: pointListData.asMap().entries.map((e) {
